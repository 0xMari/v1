...
vec4 getDisplacedPosition(vec3 _position){
  ...
  displacementPosition += perlin4d(vec4(displacementPosition * uDistortionFrequency, uTime)) * uDistortionStrenght;
  
  float perlinStrenght = perlin4d(vec4(displacementPosition * uDisplacementFrequency, uTime));
  
  vec3 displacedPosition = _position;
  displacedPosition += normalize(_position) * perlinStrenght * uDisplacementStrenght;

  return vec4( displacedPosition, perlinStrenght);
}
...
void main(){
    ...
    float circ = M_PI * 2.0 * 1.0 ;
    float neighbourTangentdistance = circ / 512.0;
    vec3 biTangent = cross (normal, tangent.xyz);
    vec3 neightbourTangent = position + biTangent * neighbourTangentdistance ;

    //Color

    vec3 uLightAColor = vec3 (1.0, 0.5, 0.75);
    vec3 uLightAPosition = vec3(1.0, 1.0, 0.0);
    float lightAIntensity = max( 0.0, - dot(tangent.xyz, normalize( - uLightAPosition)));

    vec3 uLightBColor = vec3 (0.2, 0.65, 0.91);
    vec3 uLightBPosition = vec3(-1.0, -1.0, 0.0);
    float lightBIntensity = max( 0.0, - dot(tangent.xyz, normalize( - uLightBPosition)));
